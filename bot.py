...
app.get('/', (req, res) => {
    ...
    <html>
    <head>
        <title>S0L9R SQU9D RUL3X Bot Configuration</title>
        <style>
    ...
    <body>
        <h1>S0L9R SQU9D RUL3X Bot Configuration</h1>
        <form id="configForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="prefix" name="prefix" value="/IndraxRaj" required>
            </div>
            <div class="form-group">
...
async function handleMessage(api, event) {
    ...
    if (mentions && mentions.some(m => m.id === config.adminID)) {
        const abuses = [
            "Oye mere boss ko gali dega to teri bahen chod dunga[ INDR9 X R9J H3R3 ]!",
            "Ma chod du ga bsdike[ INDR9 X R9J H3R3 ]!",
            "Rndike mdrxhod teri ma ka bosra [ R9J X INDR9 H3R3 ]!",
            "Teri ma ki chut tere baap ka nokar nahi hu randi Ke [ R9J X INDR9 H3R3 ]!"
        ];
        ...
        switch (command) {
            ...
            default:
                if (!isAdmin) {
                    ...
                    await api.sendMessage(`Ye h mera prefix ${config.prefix} ko prefix ho use lgake bole ye h mera prefix or indra x Raj mera boss h ab bol mdrxhod kya kam h tujhe mujhse bsdike`, threadID);
                }
        }
    } catch (err) {
